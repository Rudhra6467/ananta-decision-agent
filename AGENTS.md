...
- DI: `lab system` / `lab profile` / `lab di` / `lab gates` / `lab paper-sim` / `lab experiments` / `lab attribution` / `lab quality` / `lab h2` / `lab universe` / `lab memory` / `lab research`
...
**Universe v1.2:** `continuation` hist shadow. **Setup Memory v0:** `lab memory` (jsonl join). Not Wave A live. Not KEEP.  
