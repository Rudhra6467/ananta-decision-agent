"""Read latest health_sweep cards. Stock only. Not a config sweep."""
...
def _donchian(cards: List[dict]) -> Optional[dict]:
    for s in cards:
        k = _key(s)
        if "donchian" in k:
            return s
...
def print_health_card() -> Dict[str, Any]:
    ...
    card = _donchian(cards)
    keys = [_key(s) for s in cards]
    row = empty_row(
    ...
    print(f"\nHEALTH CARD  {VERSION}")
    print("=" * 64)
    print("Stock Donchian only. None is not ingest. Not KEEP.")
    print(f"  run={run_id}  meta={result_meta}")
    print(f"  strategy_keys={keys[:20]}")
    print(f"  donchian_present={card is not None}")
